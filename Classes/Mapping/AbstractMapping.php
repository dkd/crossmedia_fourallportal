...
abstract class AbstractMapping implements MappingInterface, LoggerAwareInterface
{
    ...
    protected function mapPropertyValueToObject(string $propertyName, mixed $propertyValue, AbstractEntity|File $object): bool
    {
        ...
        */
        ...
        if (str_contains($targetType, '|')) {
            $this->logger?->debug('Found union type definition "' . $targetType . '" for ' . get_class($object) . '::' . $propertyName);
            $targetTypes = explode('|', $targetType);
            $collections = array_filter(
                $targetTypes,
                fn($type) => str_contains($type, '<')
            );
        } elseif (str_contains($targetType, '<')) {
            /*
             * In order to simplify the code the target type is pushed into the collection array if it contains nested types
             * This reduce the amount of condition required later in this code
             */
         ...
         * This code uses currently the minimal effort to handle these types
         */
        if (!empty($collections)) {
            /*
             * Use the first entry collection (note that the index does not have to be 0!)
             * There is the possibility that there are multiple collections defined
             * However, the current implementation expects the corresponding collection to be the first entry
             */
            $usedTargetType = reset($collections);
                ...
                foreach ((array)$propertyValue as $identifier) {
                    ...
                    try {
                        ...
                        $this->logger?->critical(
                            vsprintf(
                                'Failed to convert identifier "%1$s" into type "%2$s" for property "%4$s" in model "%3$s". Original type: "%5$s"',
                                [
                                    $identifier,
                                    $childType,
                                    get_class($object),
            ...
            $usedTargetType = $targetType;
            /*
             * Even if a property is using union type, it is expected, that the first type should be used
             * This code needs refactoring in order to correctly handle union types
             */
            if (!empty($targetTypes)) {
                $usedTargetType = reset($targetTypes);
            }
            $sourceType = $propertyMapper->determineSourceType($propertyValue);
            $usedTargetType = trim($usedTargetType, '\\?');
            if ($usedTargetType !== $sourceType) {
                if ($usedTargetType === 'string' && $sourceType === 'array') {
                    $propertyValue = json_encode($propertyValue, JSON_HEX_QUOT | JSON_HEX_AMP | JSON_HEX_TAG);
                } else {
                    try {
                        $typeConverter = $propertyMapper->findTypeConverter($propertyValue, $usedTargetType, $configuration);
                    } catch (\Throwable $throwable) {
                        $this->logger?->critical(
                            vsprintf(
                                'Failed to convert identifier "%1$s" into type "%2$s" for property "%4$s" in model "%3$s". Original type: "%5$s"',
                    ...
                    $propertyValue = $typeConverter->convertFrom($propertyValue, $usedTargetType, [], $configuration);
                }
                ...
                if ((new ReflectionMethod(get_class($object), 'set' . ucfirst($propertyName)))->getNumberOfRequiredParameters() === 1) {
                    if (is_null($propertyValue) && is_a($usedTargetType, AbstractEntity::class, true)) {
                        return false;
                    }
                }
            }
    ...
    protected function determineDataTypeForProperty(string $propertyName, object $object): bool|string
    {
        if (property_exists(get_class($object), $propertyName)) {
            ...
            $types = [];
            $typesUsed = [];
            if (count($property->getTypes()) > 0) {
                $types = $property->getTypes();
            } else {
                $types[] = $property->getPrimaryType();
            }
            ...
            foreach ($types as $type) {
                if ($type->isCollection()) {
                    ...
                    $typesUsed[] = $type->getClassName() . '<' . implode('|', $childTypes) . '>';
                } elseif (!empty($type->getClassName())) {
                    $typesUsed[] = $type->getClassName();
                } else {
                    $typesUsed[] = $type->getBuiltinType();
                }
            }

            if (count($typesUsed) > 0) {
                return implode('|', $typesUsed);
            }
