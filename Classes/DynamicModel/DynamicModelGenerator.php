...
/**
 * This class is created dynamically. Do not edit this file since the changes will be overwritten
 */
class %s extends %s
{
%s
...
TEMPLATE;
  ...
  protected function getPropertiesString(): string
  {
      ...
      foreach ($propertyNames as $propertyName) {
        ...
        $typedProperty = $this->strictTypes ? $type . ' ' : '';
        $propertyDefinition = [
            '    /**'
        ];
  ...
  protected function registerProperty(
      ...
      if (str_contains($types, '\\Persistence\\ObjectStorage<')) {
          $this->registerUseStatement(ObjectStorage::class)
              ->registerStorageInitialization($name);
      }

      if ($lazy) {
          $this->registerUseStatement('TYPO3\CMS\Extbase\Annotation', 'Extbase');
          if ($relationType === self::RELATION_TYPE_SINGLE) {
              $this->registerUseStatement(LazyLoadingProxy::class);
              if (!str_contains($types, 'LazyLoadingProxy')) {
                  $types .= '|LazyLoadingProxy';
              }
          }
      }
  ...
  protected function registerUseStatement(string $className, string|null $alias): self
  {
