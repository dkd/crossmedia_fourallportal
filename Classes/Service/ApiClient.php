...
class ApiClient
{
    ...
    public function getModuleConfig(string $moduleName = null, bool $withRelationConf = false): array
    {
        ...
        if ($withRelationConf) {
            ...
            $relationConf = $this->buildRelationConfFromValueOptions(
                $valueOptions[$moduleName . '.referenced_by'] ?? [],
                $moduleName
            );

            $relationConf = array_merge(
                $this->buildRelationConfFromOwnFields($result['fields'], $moduleName),
                $relationConf
            );

            $result['relation_conf'] = $relationConf;
        }
    ...
    private function buildRelationConfFromOwnFields(array $fields, string $parentModule): array
    {
        $relationConf = [];
        $relationTypeFields = [
            'CEId',
        ...
        foreach ($fields as $field) {
            ...
            if (!in_array($type, $relationTypeFields, true)) {
                continue;
            }
            ...
            $childModule = $modules[0];

            $relationConf[$fieldName] = [
                'is_native'   => true,
                'name'        => $fieldName,
                'type'        => 'FIELD_LINK',
                'field'       => $fieldName,
